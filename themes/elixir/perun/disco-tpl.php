...
$this->data['head'] .= '<link rel="stylesheet" media="screen" type="text/css" href="' .
    Module::getModuleUrl('elixir/res/css/disco.css') . '" />';
...
if ($warningIsOn) {
    ...
    try {
        ...
        Logger::warning("elixir:disco-tpl: missing or invalid title or text in config-warning file");
        $warningIsOn = false;
    }
}
    ...
    if ($authContextClassRef != null) {
        foreach ($authContextClassRef as $value) {
            if (substr($value, 0, strlen(MFA_IDENTIFIER)) === MFA_IDENTIFIER) {
                Logger::info("Redirecting to " . MFA_IDP);
                $url = $this->getContinueUrl(MFA_IDP);
                HTTP::redirectTrustedURL($url);
                exit;
            } elseif (substr($value, 0, strlen(URN_CESNET_PROXYIDP_IDPENTITYID))
